...
app.post('/press-button', async (req, res) => {
	const {
		buttonIndex,
		buttonMessageId,
		buttons,
	...
	} = req.body // Get button, buttonMessageId, ref from the request body
	const token = process.env.THE_NEXT_LEG_API_TOKEN

	const buttonsIncludesButton = buttons.includes(expectedButtonString)

	// make sure that the expectedButtonString exists in the buttons array
	if (!buttonsIncludesButton) {
		res.status(400).send({
			success: false,
			error: 'expectedButtonString not found in buttons array',
		})
		return
	}

	// make sure that the buttonIndex is valid
	if (buttonIndex < 0 || buttonIndex >= buttons.length) {
		res.status(400).send({
			success: false,
			error: 'buttonIndex is invalid',
		})
		return
	}

	const data = JSON.stringify({
		button: `${expectedButtonString}`,
...
app.post('/get-imagine-progress', async (req, res) => {
	...
	getProgress(messageId, token, 0)

	console.log('get-imagine-progress', messageId)
...
async function getProgress(messageId, token, retries) {
	...
	try {
		...
		if (retries < maxRetries) {
			// If the maximum number of retries hasn't been reached yet, call the getProgress function again after a delay
			console.log(`Retry attempt number ${retries + 1}`)
			setTimeout(() => getProgress(messageId, token, retries + 1), 5000) // Increment retries counter
		} else {
...
app.post('/imagine', async (req, res) => {
	...
	const data = {
		msg: prompt + ' --v 5.2 --style raw --c 20 --stylize 200', // from the request body
	}
